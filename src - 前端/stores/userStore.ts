...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    // 登录方法，获取用户信息
    const getUserInfo = async ({user_level, account, password }: {user_level:string; account: string; password: string }) => {
      const res = await LoginAPI({user_level, account, password });
